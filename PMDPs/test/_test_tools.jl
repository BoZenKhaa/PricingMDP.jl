...
function super_simple_pp()
    ...
    β = DiscreteNonParametric([10.0], [1.0])
    B = [β]
    A = [0.0, 5.0, 10.0, 15.0]
    objective = :revenue
...
function simple_pp()
    ...
    β₁ = DiscreteNonParametric([10.0], [1.0])
    β₂ = DiscreteNonParametric([10.0, 15.0], [0.5, 0.5])
    B = [β₁, β₁, β₂]
    A = [0.0, 5.0, 10.0, 15.0]
    objective = :revenue
...
function simple_short_trace(pp::PMDPs.PMDPProblem)
    SimHistory(
        [
            (s = PMDPs.State(pp.c₀, 1, 1), info = (b = 10.0,)),
            (s = PMDPs.State(pp.c₀, 3, 2), info = (b = 10.0,)),
            (s = PMDPs.State(pp.c₀, 5, 2), info = (b = 10.0,)),
        ],
        1.0,
        nothing,
        nothing,
...
function simple_exhaust_capacity_trace(pp::PMDPs.PMDPProblem)
    ...
    trace = SimHistory(
        [
            (s = PMDPs.State(pp.c₀, 1, 1), info = (b = 10.0,)),
            (s = PMDPs.State(pp.c₀, 2, 1), info = (b = 10.0,)),
            (s = PMDPs.State(pp.c₀, 3, 2), info = (b = 10.0,)),
            (s = PMDPs.State(pp.c₀, 4, 1), info = (b = 10.0,)),
            (s = PMDPs.State(pp.c₀, 5, 1), info = (b = 10.0,)),
            (s = PMDPs.State(pp.c₀, 6, 3), info = (b = 15.0,)),
            # (s = PMDPs.State(pp.c₀, 7, 2), info = (b=10.,)),
            (s = PMDPs.State(pp.c₀, 8, 2), info = (b = 10.0,)),
        ],
        1.0,
        nothing,
        nothing,
...
function simple_trace_different_utility_revenue_optim_allocs(pp::PMDPs.PMDPProblem)
    SimHistory(
        [
            ...
            (s = PMDPs.State(pp.c₀, 2, 2), info = (b = 10.0,)),
            (s = PMDPs.State(pp.c₀, 3, 3), info = (b = 10.0,)),
            (s = PMDPs.State(pp.c₀, 4, 2), info = (b = 10.0,)),
            # (s = PMDPs.State(pp.c₀, 5, 2), info = (b=10.,)),
            (s = PMDPs.State(pp.c₀, 6, 3), info = (b = 10.0,)),
            # (s = PMDPs.State(pp.c₀, 7, 2), info = (b=10.,)),
            (s = PMDPs.State(pp.c₀, 8, 2), info = (b = 10.0,)),
        ],
        1.0,
...
function empty_trace(pp::PMDPs.PMDPProblem)
    empty_trace = SimHistory(
        [
            (
                s = PMDPs.State(pp.c₀, 1, length(pp.P) + 1),
                info = (b = PMDPs.EMPTY_PRODUCT_USER_BUDGET,),
            ),
        ],
        1.0,
        nothing,
