...
@testset "evaluation.jl" begin
    ...
    hrec = HistoryRecorder(
        max_steps = PMDPs.selling_period_end(mg),
        rng = MersenneTwister(4321),
    )

    # test replay()
    reject = FunctionPolicy(x -> 1000.0)
    ...
    @test (r, u, nₛ, nᵣ) == (0.0, 0, 0, 3)

    accept = FunctionPolicy(x -> 5.0)
    hₐ = PMDPs.replay(hrpl, accept, MersenneTwister(123))
    @test PMDPs.get_metrics(hrpl, hₐ) == (r = 15.0, u = 3, nₛ = 3, nᵣ = 3)

    # test eval()
    policies = (
        vi = PMDPs.get_VI_policy(me),
        mcts = PMDPs.get_MCTS_planner(mg, params_mcts = Dict(:rng => Xorshift128Plus(1))),
    ...
    for policy in policies
        @test PMDPs.get_metrics(hrpl, PMDPs.replay(hrpl, policy, MersenneTwister(1))) ==
              (r = 30.0, u = 3, nₛ = 3, nᵣ = 3)
    end
