...
function linear_params(objective)
    [
        (params = Dict(pairs((nᵣ=3, c=1, T=10, expected_res=6., res_budget_μ=5., 
            objective=objective))), 
         vi = true, name="linear_problem"),
        (params = Dict(pairs((nᵣ=5, c=2, T=40, expected_res=30., res_budget_μ=5., 
            objective=objective))),
         vi = true, name="linear_problem"), # last VI size to run
        (params = Dict(pairs((nᵣ=10, c=5, T=100, expected_res=100., res_budget_μ=5., 
            objective=objective))),
         vi = false, name="linear_problem"),
        (params = Dict(pairs((nᵣ=10, c=40, T=1000, expected_res=Float64(800), res_budget_μ=5., 
            objective=objective))),
         vi = false, name="linear_problem"),
...
function graph_params(objective)
    [
        (params = Dict(pairs((NV=5, NE=8, seed=1, NP=20, c=5, T=100, expected_res=Float64(80), 
            res_budget_μ=5., objective=objective))), 
         vi = false, name="graph_problem"),
        (params = Dict(pairs((NV=8, NE=20, seed=1, NP=50, c=10, T=1000, expected_res=Float64(400), 
            res_budget_μ=5., objective=objective))),
         vi = false, name="graph_problem"),
        (params = Dict(pairs((NV=15, NE=30, seed=1, NP=100, c=10, T=1000, expected_res=Float64(600), 
            res_budget_μ=5., objective=objective))),
         vi = false, name="graph_problem"),
...
function get_linear_problems(objective::Symbol)
    pps = [(pp=PMDPs.linear_pp(;p.params...), p...) 
        for p in linear_params(objective)]
end
function get_graph_problems(objective::Symbol)
    pps = [(pp=PMDPs.graph_pp(;p.params...), p...) 
        for p in graph_params(objective)]
end
