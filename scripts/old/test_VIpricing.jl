...
init_util = [value_heuristic(mdp.states[i], mdp) for i = 1:length(mdp.states)]
...
qdf = DataFrame(
    hcat(
        [repr(mdp.states[i].c) for i = 1:length(mdp.states)],
        [mdp.states[i].t for i = 1:length(mdp.states)],
        [repr(mdp.states[i].p) for i = 1:length(mdp.states)],
        policy.qmat,
    ),
